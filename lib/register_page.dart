...
class _RegisterPageState extends State<RegisterPage> {
  ...
  Future<void> _register() async {
    ...
    // 2. Ipakita ang Loading Dialog (Para hindi pindot nang pindot ang user)
    showDialog(
      context: context,
    ...
    try {
      ...
      // B. Save Profile Data sa Firestore
      // TANDAAN: Siguraduhing "users" ang collection name sa Firebase Console mo!
...
// C. MAGDAGDAG NG INITIAL INTAKE SA REALTIME DATABASE (DITO NA!)
      ...
      if (mounted) {
        ...
        // Maghintay sandali bago lumipat
        await Future.delayed(const Duration(seconds: 2));

        if (mounted) {
          // Babalik na sa LoginPage
          Navigator.pop(context); 
        }
      }
    } on FirebaseAuthException catch (e) {
      if (mounted) Navigator.pop(context); // Tanggalin ang loading
      ...
      if (mounted) Navigator.pop(context); // Tanggalin ang loading
      // DITO NATIN MAHUHULI KUNG BAKIT AYAW MAG-REGISTER
      print("DEBUG ERROR: $e"); 
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Firestore Error: Siguraduhing naka-ON ang database mo."), backgroundColor: Colors.red),
      );
    }
  }
